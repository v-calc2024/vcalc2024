...
import { getOngInfo } from "@/services/ong/getOngInfo";
...
export default function Home({ data, image, name, id, lang }: any) {
  ...
  function copyToClipboard() {
    setCopied(true);
    navigator.clipboard.writeText(`https://volunteeringimpact.eu/ngo/${id}`);
    setTimeout(() => {
      setCopied(false);
    }, 700);
  }
        ...
        <section className="profile">
          ...
          <div className="user-info">
            ...
            <div className="shared" onClick={copyToClipboard}>
              ...
              <p className="share">{copied ? copy_text : copy_shared_text}</p>
            </div>
          </div>
        </section>
        <section className="info-table-content">
          <h2>{programmes_title}</h2>
          <CustomTable headersArr={programmes} lang={lang} />
        </section>
        <section className="info-table-content">
          <h2>{activities_title}</h2>
          <CustomTable headersArr={activities} lang={lang} />
        </section>
        <section className="info-table-content">
          <h2>{volunteers_title}</h2>
          <CustomTable headersArr={volunteers} lang={lang} />
        </section>
        <section className="info-table-content">
          <h2>{sdg_title}</h2>
          <CustomTable headersArr={sdgs} lang={lang} />
        </section>
      </main>
...
export const getServerSideProps: any = async (context: any) => {
  ...
  const resp: any = await getOngInfo(token);
  if (!resp?.data) {
    return {
      redirect: {
        destination: "/",
